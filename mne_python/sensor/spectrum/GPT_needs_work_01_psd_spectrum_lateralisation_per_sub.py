...
def pick_sensor_pairs_epochspectrum(epochspectrum, right_sensor, left_sensor):
    ...
    psd_right_sensor, freqs = epochspectrum.copy().pick(picks=right_sensor).get_data(return_freqs=True)  # freqs is just for reference
...
def remove_noise_bias(lateralised_power, freqs, h_fmin, h_fmax):
    """To eliminate noise bias in sensors, this function 
    ...
    fmin and fmax: the high frequency section to use for noise bias """
...
for i, subjectID in enumerate(good_subject_pd.index):
    ...
    try:
        ...
        print(f'an error occurred while reading subject # {subjectID} - moving on to next subject')
        pass
...
# Create dataframes and remove outliers
sensor_dataframes = {}
