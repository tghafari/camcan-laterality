...
"""
====================================
C03_correlating_lateralised_spectra_substr:
    this script:
...
def working_df_maker(freq, spectra_dir, right_sensor, left_sensor, substr_lat_df, list_of_pairs):
    ...
    lat_index_dir = op.join(spectra_dir, f'{str(freq)}')
    ...
    print(f'working on {right_sensor}_{left_sensor}')
...
for freq in freqs:
    ...
    for _, row in sensors_layout_names_df.head(1).iterrows():
        print(f'working on pair {row["right_sensors"][1:8]}, {row["left_sensors"][1:8]}')

        working_df, list_of_pairs = working_df_maker(freq, spectra_dir, row["right_sensors"][1:8], row["left_sensors"][1:8], substr_lat_df, list_of_pairs)
