...
"""
====================================
C01_correlating_lateralised_bands_substr:
    this script:
    ...
    for which_pair in [0,1]:
        ...
        for pair in pairs:
            ...
            for substr in substrs:
                ls_corrs, ls_pvals = pearson_calculator(first_working_df, substr, ls_corrs, ls_pvals)

    # Preparing the lists
    ls_corrs_arr = np.asarray(ls_corrs).reshape(-1, 7)
    ls_pvals_arr = np.asarray(ls_pvals).reshape(-1, 7)
    output_pearr_df = pd.DataFrame(ls_corrs_arr, columns=['Thal', 'Caud', 'Puta', 'Pall', 'Hipp', 'Amyg', 'Accu'], index=ls_pairs)
    output_pvalue_df = pd.DataFrame(ls_pvals_arr, columns=['Thal', 'Caud', 'Puta', 'Pall', 'Hipp', 'Amyg', 'Accu'], index=ls_pairs)
